...
function App() {
	...
	const deleteNoteState = (id) => {
		...
		setNotes((prevNotes) => (prevNotes === undefined ? [] : prevNotes.filter((note) => note._id !== id)));
	};
	...
	const patchNoteState = (_id, title, content) => {
		...
		// setNotes((prevNotes) => [{ _id, title, content }, ...prevNotes.filter((note) => note._id !== _id)]);
		setNotes((prevNotes) =>
			prevNotes === undefined ? [] : prevNotes.map((note) => (note._id === _id ? { _id, title, content } : note))
		);
	};
		...
		<div className="App">
			<header className="App-header">
				<div style={dialogOpen ? AppStyle.dimBackground : {}}>
					...
					{notes && notes.length > 0 && <button onClick={deleteAllNotes}>Delete All Notes</button>}
				</div>
